...
GASTO_MAP = {"ID": "id", "Fecha": "fecha", "Evento": "evento", "Categoria": "categoria", "Descripcion": "descripcion", "MontoQ": "monto", "Responsable": "responsable", "Metodo": "metodo", "Observaciones": "observaciones"}
...
def dispatch(data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if action == "getBitacora":
            ...
            if payload.get("rol"):
                conditions.append("rol = :rol")
                params["rol"] = payload["rol"]
            where = " WHERE " + " AND ".join(conditions) if conditions else ""
            sql = f"SELECT id, fecha, usuario, email, rol, accion, detalle FROM bitacora{where} ORDER BY fecha DESC LIMIT 500"
        ...
        if action == "limpiarBitacora":
            from sqlalchemy import text
            db.execute(text("DELETE FROM bitacora"))
            db.commit()
            return {"ok": True}
            return {"ok": True}
        ...
        if action == "getFormUrl":
            ...
            url = c.valor if c else f"{_get_system_url()}/form"
            ...
            if not no_guardar:
                try:
                    ...
                    try:
                        ...
                        pdf.set_font('Helvetica','',6); pdf.set_text_color(140,150,165)
                        pdf.set_xy(mx,pdf.get_y()+6); pdf.cell(rw,4,f'{sys_url2}',0,0,'R')
                        pdf_b64=base64.b64encode(pdf.output()).decode()
